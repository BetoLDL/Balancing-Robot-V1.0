...
double last_error = 0, set_point = -0.6, present_angle, Iterm = 0 , pwm_out; // Values that need to be remembered
...
uint32_t readLeftencoder(){ // Read the encoders
  return leftCount;
}
uint32_t readRightencoder(){ // Read the encoders
  return rightCount;
}
...
double pidCalc(double angle){
  ...
  else{ // Position keeping PD controller 
    //long position_error = target_position - wheel_position;
    //Serial.print( wheel_position );
    steer_offset = -wheel_position * 0.002 + velocity * 0.01 ;
    //Serial.print("   ");
